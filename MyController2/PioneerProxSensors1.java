...
public class PioneerProxSensors1 {
  ...
  public PioneerProxSensors1(Robot r, String display_name, Pose p) {
    ...
    this.display = this.robot.getDisplay("sensor_display");
    ...
    this.display2 = this.robot.getDisplay("telemetryDisplay");
    if (this.display != null) {
      this.device_width = this.display.getWidth();
      this.device_height = this.display.getHeight();
      this.scaleFactor = Math.min(this.device_width, this.device_height)/(2*(this.maxRange+this.radius));   
  ...
  public void paint() {
    ...
    public void paint2(String moveState, Double time, Double LVelocity, Double RVelocity,Double Distance) {
    ...
    // ===================================================================================
    // draw a background
    this.display2.setColor(0xF0F0F0);     // Off White
    this.display2.fillRectangle(0, 0, this.device_width, this.device_height);
 
    // Draw Robot Body      
    this.display2.setColor(WHITE);     // White
    this.display2.fillOval(mapX(0.0),
                          mapY(0.0),
                          scale(this.radius),
                          scale(this.radius));
    
    this.display2.setColor(DARKGREY);     // Dark Grey
    this.display2.drawOval(mapX(0.0),
                          mapY(0.0),
                          scale(this.radius),
                          scale(this.radius));
                          
    // Need to indicate heading          
    this.display2.drawLine(mapX(0.0),
                          mapY(0.0),
                          mapX(Math.cos(this.robot_pose.getTheta()) * this.radius),
                          mapY(Math.sin(this.robot_pose.getTheta()) * this.radius));

    this.display2.setColor(BLACK);         // Black
    this.display2.setFont("Arial", 8, true);  // font size = 8, with antialiasing
    this.display2.drawText("Robot State: " + moveState, 1, 1);
    this.display2.drawText("Time Elapsed: " + time+" seconds", 1, 25);     
    this.display2.drawText("True Pose: "+ robot_pose, 1, 50);
    this.display2.drawText("Left wheel velocity:"+ LVelocity + " m/s",1,75);
    this.display2.drawText("Right wheel velocity:"+ RVelocity + " m/s",1,90);
    this.display2.drawText("Distance Travelled: "+ Distance + " meters",1,110);
